...
rule stringtie_count:
    ...
    output:
        directory("analysis/006_count/stringtie/{sample}/")
    conda:
    ...
    params:
        output=lambda wildcards: "analysis/006_count/stringtie/{}/{}".format(wildcards.sample, wildcards.sample),
    resources:
        mem_mb=20000
    log:
...
rule kallisto_count:
    ...
    input:
        fastq1="analysis/002_trimming/{sample}/{sample}_merged_R1_trimmed.fastq.gz",
        fastq2="analysis/002_trimming/{sample}/{sample}_merged_R2_trimmed.fastq.gz"
    output:
